...
<Card title="Echo Merchant Website" icon="link" href="https://x402.PalPaxAI.network">
...
Visit the [Echo Merchant Website](https://x402.PalPaxAI.network) to see the supported networks and endpoints for the Echo Merchant.

For example:

```
https://x402.PalPaxAI.network/api/solana-devnet/paid-content
https://x402.PalPaxAI.network/api/solana-mainnet/paid-content
https://x402.PalPaxAI.network/api/base/paid-content
https://x402.PalPaxAI.network/api/base-sepolia/paid-content
```

## Step 3 — Run and observe

Run the client. You should see the initial `402`, a retry with `X-PAYMENT`, a `200 OK` response, and a decodable `X-PAYMENT-RESPONSE` header. The tokens you sent are refunded automatically and PalPaxAI covers the fees.